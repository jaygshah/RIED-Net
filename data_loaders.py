...
class DataFolder(data.Dataset):
	def __init__(self, image_dir, image_type, transform, mode='train'):
		...
		self.mode = mode
		self.data_index = []
		self.data_urls = sorted(glob(self.image_dir+"/images/*.nii"))
		self.data_targets = sorted(glob(self.image_dir+"/targets/*.nii"))
	...
	def __getitem__(self, index):
		...
		return torch.FloatTensor(img).unsqueeze(0), torch.FloatTensor(trgt).unsqueeze(0)
...
if __name__ == '__main__':
	...
	for i, x in enumerate(loader):
		print(i, x[0].dtype, x[1].shape, torch.min(x[0]), torch.max(x[0]))
		break